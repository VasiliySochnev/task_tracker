...
class DepartmentShippingValidator:
    ...
    def __init__(self, department_field, shipping_zone_field, position_field='position'):
...
class B2BValidator:
    ...
    def __call__(self, value):
        ...
        fields_filled = all(map(is_filled, [organization_name, o_g_r_n, i_n_n, bank_account]))
        fields_empty = all(not is_filled(f) for f in [organization_name, o_g_r_n, i_n_n, bank_account])

        if client_type == "B2B" and not fields_filled:
            raise serializers.ValidationError("Все поля для B2B клиентов должны быть заполнены.")
        elif client_type == "B2C" and not fields_empty:
            raise serializers.ValidationError("Поля организации не должны быть заполнены для B2C клиентов.")
